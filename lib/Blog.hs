...
instance (Monad m, MonadIO m, MonadReader r m, HasDbConnPool r) => BlogQuery m where
  findBlogEntries =
    withConnM Query.findBlogEntries
  findBlogEntryById blogEntryId =
    withConnM $ \conn -> Query.findBlogEntryById conn blogEntryId

instance (Monad m, MonadIO m, MonadReader r m, HasDbConnPool r) => BlogCommand m where
